...
class TestSceneRendererPanelShapeProcessorContract(unittest.TestCase):
    ...
    def test_non_back_panel_skips_processor(self):
        renderer, _doc = self._make_renderer(
            [
                SimpleNamespace(
                    kind="back_panel_groove",
        ...
        with patch.object(self.renderer, "process_panel_shape") as processor_spy:
            renderer._render_simple_panel(node)

        processor_spy.assert_not_called()
