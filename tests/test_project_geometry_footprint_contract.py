...
class TestProjectGeometryFootprintContract(unittest.TestCase):

    def test_adr_002_and_adr_003_define_footprint_as_projection_of_envelope(self):
        adr_002 = Path(
        ...
        for token in (
            ...
            "2D projection of the envelope onto the project XY plane",
    ...
    def test_furniture_project_and_cabinet_placement_remain_unchanged(self):
        ...
        project = FurnitureProject(
            project_id="P-GEO-FP-1",
            name="Footprint Boundary Project",
            cabinets=["CAB-1"],
    ...
    def test_footprint_is_not_owned_by_layout_scenegraph_manufacturing_or_cost(self):
        ...
        self.assertNotIn("footprint", layout_sources)
        ...
        self.assertNotIn("footprint", scene_graph_sources)
        ...
        self.assertNotIn("footprint", ownership_sources)

    def test_footprint_is_documented_as_derived_from_envelope_not_equal_to_bounds_or_envelope(self):
        adr_002_text = Path(
            "docs/architecture/ADR-002-canonical-project-spatial-conventions.md"
        ).read_text(encoding="utf-8")

        self.assertIn("Footprint is the project-floor projection of that extent.", adr_002_text)
        self.assertIn("2D projection of the envelope onto the project XY plane", adr_002_text)
        self.assertIn("Envelope is the full 3D occupied extent.", adr_002_text)
        self.assertNotIn("Footprint is the same as bounds", adr_002_text)
        self.assertNotIn("Footprint is the same as envelope", adr_002_text)

    def test_no_footprint_implementation_or_algorithm_exists(self):
        production_source = "\n".join(
            inspect.getsource(module)
            for module in (
                __import__("domain.furniture_project", fromlist=["FurnitureProject"]),
                __import__("domain.topology", fromlist=["BoundingBox"]),
        ...
        for token in (
            ...
            with self.subTest(footprint_algorithm_token=token):
                self.assertNotIn(token, production_source)

        self.assertNotRegex(production_source, re.compile(r"\bclass\s+Footprint\b"))
        self.assertNotRegex(
            production_source, re.compile(r"\bdef\s+[A-Za-z_]*footprint[A-Za-z_]*\b")
        )
