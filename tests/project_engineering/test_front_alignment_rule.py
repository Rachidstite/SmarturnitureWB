...
class TestFrontAlignmentRule(unittest.TestCase):
    ...
    def _drawer_face(self, name, x_mm, y_mm=10.0, z_mm=100.0, width_mm=400.0, height_mm=200.0):
    ...
    def test_passes_when_offsets_are_within_tolerance(self):
        ...
        model = replace(
            ...
            doors=(
                ...
                self._door("SEC-1_Door_2", 18.3),
            ),
            drawer_faces=(),
        )

        report = FrontAlignmentRule.evaluate(model, tolerance_mm=0.5)
        ...
        self.assertGreaterEqual(report.decision.aligned_component_count, 2)
        self.assertEqual(len(report.violations), 0)

    def test_warns_when_offsets_exceed_tolerance_but_remain_close(self):
        from project_engineering.front_alignment_rule import FrontAlignmentRule
    ...
    def test_fails_when_offsets_exceed_the_fail_threshold(self):
        from project_engineering.front_alignment_rule import FrontAlignmentRule
        ...
        model = replace(
            ...
            doors=(
                self._door("SEC-1_Door_1", 18.0),
                self._door("SEC-1_Door_2", 20.0),
            ),
            drawer_faces=(
                self._drawer_face("SEC-1_DrawerFace_1", 18.0),
            ),
        )
    ...
    def test_rule_module_has_no_geometry_engine_or_scene_graph_dependency(self):
        module = importlib.import_module("project_engineering.front_alignment_rule")
        source = inspect.getsource(module)

        self.assertIn("FrontAlignmentRule", source)
