...
class TestStaticDoorDrawerCollisionFact(unittest.TestCase):
    ...
    def test_extracts_facts_from_doors_and_drawer_faces(self):
        ...
        facts = StaticDoorDrawerCollisionRule.extract_facts(model)

        self.assertEqual(len(facts), 2)
        self.assertEqual(
            {fact.component_type for fact in facts},
            {"DOOR_PANEL", "DRAWER_FACE"},
        )
        self.assertTrue(all(fact.source.startswith("resolved_") for fact in facts))
        self.assertTrue(all(fact.thickness_mm > 0 for fact in facts))

    def test_fact_module_has_no_geometry_engine_or_scene_graph_dependency(self):
        module = importlib.import_module(
            "project_engineering.static_door_drawer_collision_fact"
        )
        source = inspect.getsource(module)

        self.assertIn("StaticDoorDrawerCollisionFact", source)
