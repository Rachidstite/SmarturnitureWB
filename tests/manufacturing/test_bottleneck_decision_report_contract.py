...
def test_bottleneck_decision_report_contract_exists():
    from manufacturing.bottleneck_decision_report import BottleneckDecisionReport

    assert is_dataclass(BottleneckDecisionReport)


def test_bottleneck_decision_report_field_inventory_is_stable():
    from manufacturing.bottleneck_decision_report import BottleneckDecisionReport

    assert [field.name for field in fields(BottleneckDecisionReport)] == [
        "decision_status",
        "is_manufacturable",
        "is_blocked",
        "requires_review",
...
def test_bottleneck_decision_report_safe_defaults():
    from manufacturing.bottleneck_decision_report import BottleneckDecisionReport

    report = BottleneckDecisionReport()
    ...
    assert report.bottleneck_area == ""
    assert report.factory_visibility_message == ""


def test_bottleneck_decision_report_has_no_runtime_or_builder_logic():
    import manufacturing.bottleneck_decision_report as module
...
def test_bottleneck_decision_report_does_not_change_runtime_behavior():
