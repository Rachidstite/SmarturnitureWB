...
def test_capacity_reservation_decision_report_contract_exists():
    from manufacturing.capacity_reservation_decision_report import (
        CapacityReservationDecisionReport,
    )

    assert is_dataclass(CapacityReservationDecisionReport)


def test_capacity_reservation_decision_report_field_inventory_is_stable():
    from manufacturing.capacity_reservation_decision_report import (
        CapacityReservationDecisionReport,
    )

    assert [field.name for field in fields(CapacityReservationDecisionReport)] == [
        "decision_status",
        "is_manufacturable",
        "is_blocked",
        "requires_review",
        ...
        "warning_reason",
        "recommended_fix",
        "capacity_reservation_action",
        "factory_visibility_message",
    ]


def test_capacity_reservation_decision_report_safe_defaults():
    from manufacturing.capacity_reservation_decision_report import (
        CapacityReservationDecisionReport,
    )

    report = CapacityReservationDecisionReport()
    ...
    assert report.capacity_reservation_action == ""
    assert report.factory_visibility_message == ""


def test_capacity_reservation_decision_report_has_no_runtime_or_builder_logic():
    import manufacturing.capacity_reservation_decision_report as module
    ...
    assert "utilization" not in source.lower()
...
def test_capacity_reservation_decision_report_does_not_change_runtime_behavior():
