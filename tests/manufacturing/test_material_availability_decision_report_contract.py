...
def test_material_availability_decision_report_contract_exists():
    from manufacturing.material_availability_decision_report import (
        MaterialAvailabilityDecisionReport,
    )

    assert is_dataclass(MaterialAvailabilityDecisionReport)


def test_material_availability_decision_report_field_inventory_is_stable():
    from manufacturing.material_availability_decision_report import (
        MaterialAvailabilityDecisionReport,
    )

    assert [field.name for field in fields(MaterialAvailabilityDecisionReport)] == [
        "decision_status",
        "is_manufacturable",
        "is_blocked",
        "requires_review",
...
def test_material_availability_decision_report_safe_defaults():
    from manufacturing.material_availability_decision_report import (
        MaterialAvailabilityDecisionReport,
    )

    report = MaterialAvailabilityDecisionReport()
    ...
    assert report.material_availability_status == ""
    assert report.factory_visibility_message == ""


def test_material_availability_decision_report_has_no_runtime_or_builder_logic():
    import manufacturing.material_availability_decision_report as module
    ...
    assert "procurement" not in source.lower()
...
def test_material_availability_decision_report_does_not_change_runtime_behavior():
