...
# Validation Review Wiring Contract Tests
#
# Verifies:
# - refresh_validation wires source through build_validation_review_projection
# - outputs ReviewPanelReadModel with validation facts as UI-safe rows
...
class TestValidationReviewWiringContract(unittest.TestCase):
    """Verify validation review wiring in service integration."""
    ...
    # ── Rule 1: wires validation source through adapter ─────────

    def test_wires_validation_source_through_adapter(self):
        """refresh_validation routes source to build_validation_review_projection."""
        ...
        source = _FakeValSource([
            ...
            _FakeValViolation("Height Check", "WARNING", "WARNING", "Near limit"),
        ])
        result = integration.refresh_validation(source)

        self.assertIsInstance(result, tuple)
        val_panel = [p for p in result if p.panel_name == "Validation"][0]
        self.assertTrue(val_panel.available)
        section_names = {s.section_name for s in val_panel.sections}
    ...
    def test_returns_tuple_of_review_panel_read_models(self):
        ...
        source = _FakeValSource([_FakeValViolation("Rule 1", "ERROR", "FAIL")])
        result = integration.refresh_validation(source)

        for panel in result:
            self.assertIsInstance(panel, rm.ReviewPanelReadModel)

    # ── Rule 3: output contains validation facts as UI-safe rows ─

    def test_output_contains_validation_facts_as_ui_safe_rows(self):
        """Validation fact rows are (str, str) tuples — no raw objects."""
        rm, pa, ws, si = self._import_modules()
        ...
        source = _FakeValSource([
            _FakeValViolation("Check A", "ERROR", "FAIL", "Tolerance exceeded"),
        ])
        result = integration.refresh_validation(source)

        val_panel = [p for p in result if p.panel_name == "Validation"][0]
        for section in val_panel.sections:
    ...
    # ── Rule 4: no raw validation object leaks ──────────────────

    def test_no_raw_validation_objects_leak(self):
        """Workspace stores no raw validation source references."""
        rm, pa, ws, si = self._import_modules()
        ...
        source = _FakeValSource([_FakeValViolation("Rule A", "ERROR", "FAIL")])
        integration.refresh_validation(source)
    ...
    def test_other_refresh_methods_unchanged(self):
        """refresh_cost_review, refresh_commercial_review, etc. still delegate to _refresh_review_panels."""
        rm, pa, ws, si = self._import_modules()
    ...
    def test_none_source_falls_back_safely(self):
        ...
        result = integration.refresh_validation(None)

        self.assertIsInstance(result, tuple)
        self.assertGreaterEqual(len(result), 5)

    def test_empty_violations_falls_back_safely(self):
        """Empty violations source returns available=False validation panel."""
        rm, pa, ws, si = self._import_modules()
        ...
        self.assertIsInstance(result, tuple)
        val_panel = [p for p in result if p.panel_name == "Validation"][0]
        self.assertFalse(val_panel.available)
    ...
    def test_deterministic(self):
        ...
        r1 = integration.refresh_validation(source)
        r2 = integration.refresh_validation(source)
    ...
    def test_no_forbidden_imports(self):
        """service_integration.py must not import domain/validation/etc modules."""
    ...
    def test_no_engine_workflow_naming(self):
        """No engine/workflow naming in refresh_validation method name."""
        rm, pa, ws, si = self._import_modules()

        method = si.ConfiguratorV2ServiceIntegration.refresh_validation
