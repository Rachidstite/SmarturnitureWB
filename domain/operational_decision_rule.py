...
def apply_operational_decision_rule(
    ...
    if normalized.status in {"BLOCKED", "WARNING", "NEEDS_REVIEW", "RISK"}:
        note_parts = ["Operational evidence", normalized.status]
        ...
        if len(note_parts) > 2:
            manufacturing_impact_note += " | " + " | ".join(note_parts[2:])
        if messages:
            manufacturing_impact_note += " | " + " | ".join(messages)
